...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <form method="get" action="{{ url('/playerReport') }}">
                        ...
                        <table id="dataTableExample" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                ...
                                @if (isset($_GET['username']))
                                    {{--  {{ $payment }}  --}}
                                    {{--  @dd($payment, Session::get('role'));  --}}
                                    @foreach ($payment['sum'] ?? [] as $index => $pay)
                                        <tr>
                                            <td>{{ $index + 1 }}</td>
                                            <td>{{ $payment['user'][0]['userName'] ?? '' }}</td>
                                            <td>{{ $pay['DrDate'] ?? 'No date' }} </td>
                                            <td>
                                                <b>Bet</b>: {{ $pay['bet'] ?? 0 }}
                                            </td>
                                            <td>
                                                <b>Won</b>: {{ $pay['won'] ?? 0 }}<br>
                                            </td>
                                            {{--  <td>
                                                <b>EndPoint</b>: {{ $pay['endPoint'] ?? 0 }}
                                            </td>  --}}
                                            <td>
                                                <b>Profit</b>: {{ ($pay['won'] ?? 0) - ($pay['bet'] ?? 0) }}<br>
                                            </td>
                                    ...
                                    @if (!empty($_GET['username']))
                                        ...
                                        @endforeach  --}}
                                            <td>
                                                <b>Profit</b>:
                                                {{ isset($payment['totalProfit']) ? $payment['totalProfit'] : 0 }}
                                                <br>
                                                {{--  <b>End Point</b>: {{ $payment['totalEndPoint'] ?? 0 }}  --}}
                                            </td>
                                            <td>
                                                {{--  @dd($pay)  --}}
                                                <form action="{{ route('genrep.show', ['id' => $_GET['username']]) }}"
                                                    method="get">
                                ...
                                @if (empty($_GET['username']))
                                    {{--  @dd($userGroup);  --}}
                                    {{--  {{$userGroup['userName']}}  --}}
                                    @foreach ($userGroup as $index => $pay)
                                        {{--  @dd($userGroup);  --}}
                                        <tr>
                                            <td>{{ $index + 1 }}</td>
                                            <td>{{ $pay['userName'] }}</td>
                                            {{-- <td>{{ formatMongoDate($pay['createdAt']) }} </td> --}}
                                            {{--  @dd($pay['DrDate']);  --}}
                                             <td>{{ $pay['createdAt'] }}</td>
                                            <td>
                                                Bet: <b>{{ $pay['totalBet'] ?? 0 }}</b>
                                            </td>
                                            <td>
                                                Won: <b>{{ $pay['totalWon'] ?? 0 }}</b>
                                            </td>
                                            {{--  <td>
                                                EndPoint: <b>{{ $pay['totalEndPoint'] ?? 0 }}</b>
                                            </td>  --}}
                                            <td>
                                                Profit: <b>{{ ($pay['totalWon'] ?? 0) - ($pay['totalBet'] ?? 0) }}</b>
                                            </td>
                                            <td>
                                                <form action="{{ route('genrep.show', ['id' => $pay['playerId']]) }}"
                                                    method="get">
                                                    <button class="btn btn-primary"><i class="fa fa-print"
                                                            aria-hidden="true"></i>
                                                    </button>
